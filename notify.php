...
if($result===true && $_POST['trade_status'] == 'TRADE_SUCCESS'){
    ...
    mysqli_query($conn,"UPDATE `content` SET `ispaid`='true' WHERE `outTradeNo`='$trade'");
    $arr = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM `content` WHERE `outTradeNo`='$trade'"));
    $comment = $arr['comment'];
    $money = $arr['money'];
    $time = $arr['time'];
    
    mysqli_query($conn,"INSERT INTO `order` VALUES('$trade','$money','$comment','$time')");
